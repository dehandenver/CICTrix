...
Fix (not applied — it is a design decision): vary `required_proficiency` by
position level, so a Supervising Administrative Officer is held to 4–5 rather
than 3. That produces genuine gaps at the top of the org without lowering
anyone's ratings. Lowering seeded ratings instead would reintroduce the
"Summary of Ratings reads low" problem.
