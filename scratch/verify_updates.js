...
async function verify() {
  ...
  console.log(`Starting verification of ${lines.length} employees...`);
  ...
  for (const line of lines) {
    ...
    if (empErr || !emp) {
      console.error(`❌ Verification failed: Employee ${empNumber} not found or query error:`, empErr);
      failCount++;
      continue;
    }

    // Query employee_portal_accounts table
    ...
    if (accErr || !acc) {
      console.error(`❌ Verification failed: Portal account for ${empNumber} not found or query error:`, accErr);
      failCount++;
      continue;
    }
    ...
    // Validate fields
    const checks = [
      { name: 'email', expected: email, actual: emp.email },
      { name: 'phone', expected: phone, actual: emp.phone },
      { name: 'address', expected: address, actual: emp.current_address_street },
      { name: 'dob', expected: dob, actual: emp.date_of_birth },
      { name: 'pob', expected: pob, actual: emp.place_of_birth },
      { name: 'sex', expected: sex, actual: emp.sex },
      { name: 'civilStatus', expected: civilStatus, actual: emp.civil_status },
      { name: 'position', expected: position, actual: emp.position },
      { name: 'department', expected: department, actual: emp.department },
      { name: 'dateHired', expected: dateHired, actual: emp.date_hired },
      { name: 'emergencyContactName', expected: emergencyContact, actual: emp.emergency_contact_name },
      { name: 'emergencyRelationship', expected: emergencyRelationship, actual: emp.emergency_contact_relationship },
      { name: 'emergencyContactPhone', expected: emergencyPhone, actual: emp.emergency_contact_phone },
      { name: 'portalUsername', expected: username, actual: acc.username },
      { name: 'portalPassword', expected: password, actual: acc.password },
      { name: 'portalFullName', expected: fullName, actual: acc.full_name },
      { name: 'portalEmail', expected: email, actual: acc.email },
      { name: 'portalPhone', expected: phone, actual: acc.mobile_number },
      { name: 'employmentStatus', expected: 'Regular', actual: emp.employment_status },
    ];

    let matches = true;
    for (const check of checks) {
      if (check.expected !== check.actual) {
        console.error(`❌ Mismatch for ${empNumber} (${fullName}) field '${check.name}': Expected '${check.expected}', got '${check.actual}'`);
        matches = false;
      }
    }
  ...
  console.log(`✅ Successfully verified: ${successCount}/${lines.length}`);
  console.log(`❌ Failed verification: ${failCount}/${lines.length}`);
}
