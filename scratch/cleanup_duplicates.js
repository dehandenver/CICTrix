...
53	Nenita Garcia Tolentino	nenita.tolentino@company.com.ph	0998-402-6549	898 Luna St., Barangay Guadalupe, Tagum City	December 05, 1973	Lipa City	52	Female	Married	EMP-2026-014	IT Support Specialist	Warehouse and Logistics	March 26, 2026	Active	Keep_Nenita	Brother	0918-244-4696	ntolentino	3RURz92Z%56
...
async function cleanup() {
  ...
  console.log(`Starting cleanup of duplicate portal accounts...`);
  ...
  for (const line of lines) {
    ...
    // Get all accounts with this employee_id
    ...
    if (error) {
      console.error(`Error querying accounts for ${empNumber}:`, error);
      continue;
    }

    if (accounts.length > 1) {
      console.log(`Found ${accounts.length} accounts for ${empNumber} (${fullName}). cleaning up...`);
      // We want to keep the one that matches our target username
      const correctAccount = accounts.find(a => a.username === username);
      
      if (correctAccount) {
        // Delete all other accounts
        const idsToDelete = accounts
          .filter(a => a.id !== correctAccount.id)
          .map(a => a.id);
        
        console.log(`Keeping account ID: ${correctAccount.id} (username: ${correctAccount.username}). Deleting IDs:`, idsToDelete);
        ...
        if (delErr) {
          console.error(`Error deleting duplicates for ${empNumber}:`, delErr);
        } else {
          console.log(`✅ Duplicates deleted successfully for ${empNumber}.`);
        }
      } else {
        // None of the accounts have the correct username. We will keep the first one and rename its username/password, and delete the rest.
        const primaryAccount = accounts[0];
        const idsToDelete = accounts.slice(1).map(a => a.id);
        
        console.log(`Renaming username of account ${primaryAccount.id} to ${username} and keeping it. Deleting IDs:`, idsToDelete);
        ...
        if (updateErr) {
          console.error(`Error updating username for ${empNumber}:`, updateErr);
        }
        ...
        console.log(`✅ Cleaned up and updated account for ${empNumber}.`);
      }
    }
  }
