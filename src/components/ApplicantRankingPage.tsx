...
@media print {
  ...
  .print-pos   { font-size: 13pt; font-weight: 700; margin: 14px 0 4px; border-bottom: 2px solid #000; padding-bottom: 3px; }
}
...
export const ApplicantRankingPage = () => {
  ...
  const groups: PositionGroup[] = Object.entries(
    ...
    .map(([position, members]) => {
      ...
      return {
        position,
        office:   members.find(m => m.office)?.office ?? '—',
        members:  sorted,
  ...
  const selectAll = () => {
    ...
    setSelected(new Set(
      activeGroup.members.filter(a => !a.status.toLowerCase().includes('hired')).map(a => a.id)
    ));
  };
      ...
      <div className="min-h-screen bg-[#f8f9fa]">
        ...
        <div className="admin-layout">
          ...
          <main className="admin-content bg-slate-50 !p-0">
            ...
            {/* ── VIEW 1 — Positions overview ─────────────────────────── */}
            {!loading && !activePosition && (
              <div className="p-6">
                ...
                {/* Screen table */}
                <div className="no-print overflow-hidden rounded-2xl border border-slate-200 bg-white">
                        ...
                        >
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-2.5">
                              <div className="shrink-0 rounded-lg bg-amber-100 p-1.5 text-amber-600">
                                <Trophy size={14} />
                              </div>
                      ...
                      {groups.length === 0 && (
                        <tr>
                          <td colSpan={6} className="px-5 py-12 text-center text-slate-400">
                            ...
                            <p className="font-medium text-sm">No applicant data yet. Scores appear once evaluations are completed.</p>
                          </td>
                        </tr>
                      )}
                    </tbody>
                ...
                <div id="ranking-print-area">
                  <p className="print-show print-title">APPLICANT RANKING REPORT</p>
                  <p className="print-show print-sub">
                    Office of the City Human Resource Management Officer — ABYAN HRIS
                    <br />Printed: {printedDate}
                  </p>
                  {groups.map(g => (
                    <div key={g.position} className="print-show">
                      <p className="print-pos">{g.position} — {g.office} ({g.members.length} applicant{g.members.length !== 1 ? 's' : ''})</p>
                      <table>
            ...
            {!loading && activePosition && (
              <div className="p-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        {activeGroup?.office} · {activeGroup?.members.length} applicant{activeGroup?.members.length !== 1 ? 's' : ''}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                        ...
                        <th className="w-12 px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">
                          <input
                            ...
                            checked={
                              selected.size > 0 &&
                              activeGroup !== null &&
                              selected.size === activeGroup.members.filter(a => !a.status.toLowerCase().includes('hired')).length
                            }
                        ...
                        return (
                          <tr
                            ...
                            className={`border-b border-slate-100 last:border-0 transition-colors ${
                              isSelected ? 'bg-blue-50' : isHired ? 'bg-emerald-50/50' : 'hover:bg-slate-50'
                            }`}
                          >
                            <td className="px-4 py-3.5 text-center">
                              <input
                                ...
                                className="h-4 w-4 rounded border-slate-300 accent-[#363EE8] disabled:cursor-not-allowed disabled:opacity-40"
                              />
                            </td>
                            <td className="px-4 py-3.5 text-center">
                              <span className={`inline-flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${medalStyle(rank)}`}>
                                {rank}
                              </span>
                            </td>
                            ...
                            <td className="px-4 py-3.5">
                              <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${
                                (a.application_type ?? '').toLowerCase().includes('promot')
                ...
                <div id="ranking-print-area">
                  <p className="print-show print-title">APPLICANT RANKING — {activePosition}</p>
                  <p className="print-show print-sub">
                  ...
                  <table className="print-show">
                    <thead>
                      <tr><th>Rank</th><th>Applicant Name</th><th>Office</th><th>Type</th><th>Score</th><th>Status</th></tr>
                    </thead>
                    <tbody>
