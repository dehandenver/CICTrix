...
import { ArrowUpDown, ChevronDown, GraduationCap, RefreshCw } from 'lucide-react';
import { useCallback, useEffect, useRef, useState } from 'react';
import { useRealtimeRefresh } from '../../../hooks/useRealtimeRefresh';
import { EmptyState } from '../../../components/EmptyState';
import { getAdjectival, type IPCRRatingRecord } from './SummaryOfRatings';
...
export const SemesterSummarySection = () => {
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-indigo-200 bg-indigo-50/30 p-6">
      ...
      {/* Employee ratings for the new semester */}
      <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-gray-100 bg-gray-50 px-5 py-3">
          <span className="text-[11px] font-semibold uppercase tracking-wide text-gray-400">
            {sorted.length} employee{sorted.length !== 1 ? 's' : ''} rated this semester
          </span>
          <button
            type="button"
